...
if(typeof(cancelSortButton)!="undefined"){cancelSortButton.click(function(event){event.preventDefault();cancelSortRows(event)})};function saveRow(event,recursive){if(typeof(recursive)=="undefined"){recursive=!1}
...
applyHook('deleteNewRowDone',{"table":table})};function editModal(event){var $row=$(event.target).closest('tr');var $modal=$('#edit-modal');var id=$row.attr('data-id');$.blockUI({message:''});var request=$.ajax({method:"GET",url:urls.urlAjaxEditableModalGet.replace("_id_",id),cache:!1});request.done(function(html){$modal.html(html);validationResetFields($modal.find('.value-field'));activateJs($modal);$modal.trigger('create');validationAddFields($modal.find('.value-field'));focusInput($modal);applyHook('editModalRequestDone',{"$modal":$modal});$.unblockUI()});request.fail(function(jqXHR,textStatus){$.unblockUI();alert("Request failed: "+textStatus)})}
