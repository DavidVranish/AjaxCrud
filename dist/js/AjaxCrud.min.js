...
var editAllButton=config.editAllButton
var saveAllButton=config.saveAllButton
var sortButton=config.sortButton;var cancelSortButton=config.cancelSortButton;var formValidation;var dataTable=table.DataTable(config.datatableArgs);var sortMode='view';var urls={urlAjaxRowGet:config.urls.urlAjaxRowGet,urlAjaxRowPut:config.urls.urlAjaxRowPut,urlAjaxEditableRowGet:config.urls.urlAjaxEditableRowGet,urlAjaxRowsPut:config.urls.urlAjaxRowsPut,urlAjaxEditableRowsGet:config.urls.urlAjaxEditableRowsGet,urlAjaxNewRowGet:config.urls.urlAjaxNewRowGet,urlAjaxRowDelete:config.urls.urlAjaxRowDelete,urlAjaxRowsPost:config.urls.urlAjaxRowsPost,urlAjaxModalPut:config.urls.urlAjaxModalPut,urlAjaxModalPost:config.urls.urlAjaxModalPost,urlAjaxNewModalGet:config.urls.urlAjaxNewModalGet,urlAjaxEditableModalGet:config.urls.urlAjaxEditableModalGet,urlAjaxSortPut:config.urls.urlAjaxSortPut};table.on('draw.dt',function(){table.find('tbody td[data-colspan]').each(function(index,element){var colspan=$(element).attr('data-colspan');if($(element).attr('colspan')!=colspan){for(i=0;i<(colspan-1);i++){$(element).next().remove()}
$(element).attr('colspan',colspan)}});if(typeof(dataTable)!="undefined"&&typeof(sortButton)!="undefined"){if($.trim(dataTable.search())==""){sortButton.show()}else{sortButton.hide()}}});dataTable.draw();table.find('tbody td').each(function(index,element){if(typeof($(element).attr('data-order'))=='undefined'||$(element).attr('data-order').length<1){$(element).attr('data-order',$(element).text())}});if(typeof(form)!="undefined"){formValidation=form.data('formValidation');if($('[id='+form.attr('id')+']').length>1){console.warn('Form Validation will have problems, duplicate IDs detected!')}}
...
function activateJs($parent){if(typeof(App)!="undefined"&&typeof(App.activate)!="undefined"){App.activate($parent)}};if(typeof(form)!="undefined"){form.on('changeDate','.value-field',function(event){if(typeof(formValidation)!="undefined"){formValidation.revalidateField($(event.target))}})}
...
if(typeof(editAllButton)!="undefined"){editAllButton.click(function(event){event.preventDefault();editAllRows(event)})}
if(typeof(saveAllButton)!="undefined"){saveAllButton.click(function(event){event.preventDefault();saveAllRows(event)})};function saveRow(event,recursive){if(typeof(recursive)=="undefined"){recursive=!1}
...
var id=applyFilter('saveRowId',$row.attr('data-id'),{'$row':$row});var data={};$row.find('.value-field').each(function(index,element){if(!($(element).is(':checkbox'))||$(element).is(':checked')){data[$(element).attr('name')]=$(element).val()}});data=applyFilter('saveRowData',data,{'$row':$row,'id':id});var url=applyFilter('saveRowUrl',urls.urlAjaxRowPut.replace("_id_",id),{'$row':$row,'id':id,'data':data});var request=$.ajax({method:"PUT",url:url,cache:!1,data:data});request.done(function(html){validationResetFields($row.find('.value-field'));var $rowIndex=dataTable.row($row).index();revertColspan($row);$html=$($.parseHTML($.trim(html)));$dRow=dataTable.row.add($html);dataTable.context[0].aoData[$rowIndex]=dataTable.context[0].aoData[$dRow.index()];$dRow.remove();dataTable.draw(!1);$row=$(dataTable.row($rowIndex).node());applyHook('saveRowRequestDone',{"$row":$row});activateJs($row);$.unblockUI()});request.fail(function(jqXHR,textStatus){$.unblockUI();alert("Request failed: "+textStatus)})}
function editRow(event){var $row=$(event.target).closest('tr');var id=$row.attr('data-id');var data={};data=applyFilter('editRowDataFilter',data,{'$row':$row});$.blockUI({message:''});var request=$.ajax({method:"GET",url:urls.urlAjaxEditableRowGet.replace("_id_",id),data:data,cache:!1});request.done(function(html){var $row=$(event.target).closest('tr');var $rowIndex=dataTable.row($row).index();validationResetFields($row.find('.value-field'));$html=$($.parseHTML($.trim(html)));$dRow=dataTable.row.add($html);dataTable.context[0].aoData[$rowIndex]._aData=dataTable.context[0].aoData[$dRow.index()]._aData;dataTable.context[0].aoData[$rowIndex].anCells=dataTable.context[0].aoData[$dRow.index()].anCells;dataTable.context[0].aoData[$rowIndex].nTr=dataTable.context[0].aoData[$dRow.index()].nTr;$dRow.remove();dataTable.draw(!1);$row=$(dataTable.row($rowIndex).node());activateJs($row);validationAddFields($row.find('.value-field'));focusInput($row);applyHook('editRowRequestDone',{"$row":$row});$.unblockUI()});request.fail(function(jqXHR,textStatus){$.unblockUI();alert("Request failed: "+textStatus)})}
function cancelRow(event){var $row=$(event.target).closest('tr');var id=$row.attr('data-id');var data={};data=applyFilter('cancelRowDataFilter',data,{'$row':$row});$.blockUI({message:''});var request=$.ajax({method:"GET",url:urls.urlAjaxRowGet.replace("_id_",id),data:data,cache:!1});request.done(function(html){var $rowIndex=dataTable.row($row).index();validationResetFields($row.find('.value-field'));revertColspan($row);$html=$($.parseHTML($.trim(html)));$dRow=dataTable.row.add($html);dataTable.context[0].aoData[$rowIndex]._aData=dataTable.context[0].aoData[$dRow.index()]._aData;dataTable.context[0].aoData[$rowIndex].anCells=dataTable.context[0].aoData[$dRow.index()].anCells;dataTable.context[0].aoData[$rowIndex].nTr=dataTable.context[0].aoData[$dRow.index()].nTr;$dRow.remove();dataTable.draw(!1);$row=$(dataTable.row($rowIndex).node());activateJs($row);applyHook('cancelRowRequestDone',{"$row":$row});$.unblockUI()});request.fail(function(jqXHR,textStatus){$.unblockUI();alert("Request failed: "+textStatus)})}
function deleteRow(event){var id=$(event.target).attr('data-id');if(typeof(id)=="undefined"){id=$(event.target).closest('a').attr('data-id')}
var data={};data=applyFilter('deleteRowDataFilter',data,{'id':id,event:event});$.blockUI({message:''});var request=$.ajax({method:"DELETE",url:urls.urlAjaxRowDelete.replace("_id_",id),data:data,cache:!1});request.done(function(html){var $row=applyFilter('deleteRowRowFilter',table.find('tr[data-id="'+id+'"]'),{'id':id,event:event});dataTable.row($row).remove().draw(!1);applyHook('deleteRowDone',{});$.unblockUI()});request.fail(function(jqXHR,textStatus){$.unblockUI();alert("Request failed: "+textStatus)})}
...
function editAllRows(event){var data={};data=applyFilter('editAllRowsDataFilter',data,{});$.blockUI({message:''});var request=$.ajax({method:"GET",url:urls.urlAjaxEditableRowsGet,data:data,cache:!1});request.done(function(html){dataTable.clear();if(typeof(saveAllButton)!="undefined"){saveAllButton.show()}
if(typeof(editAllButton)!="undefined"){editAllButton.hide()}
var rows=$.parseHTML($.trim(html));$(rows).each(function(index,element){if($(element).context.nodeName!="#text"){var $row=$(dataTable.row.add($(element)).draw(!1).node());activateJs($row);validationAddFields($row.find('.value-field'))}});focusInput(table.find('tr:first'));applyHook('editAllRowsRequestDone',{});$.unblockUI()});request.fail(function(jqXHR,textStatus){$.unblockUI();alert("Request failed: "+textStatus)})}
function saveAllRows(event){if(typeof(recursive)=="undefined"){recursive=!1}
var $tbody=table.find('tbody');if(recursive==!1){$.blockUI({message:''})}
if(validationIsValid($tbody,recursive)==null){setTimeout(function(){saveRow(event,!0)},150);return !1}else if(validationIsValid($tbody,recursive)==!1){return !1}
$.blockUI({message:''});var rowsData={};table.find('tbody tr.edit-row').each(function(index,element){var rowData={};$(element).find('.value-field').each(function(index,element){if(!$(element).is(':checkbox')||$(element).is(':checked')){rowData[$(element).attr('name')]=$(element).val()}});rowsData[index]=rowData});var request=$.ajax({method:"PUT",url:urls.urlAjaxRowsPut,data:{rows:rowsData},cache:!1});request.done(function(html){table.find('tbody tr.edit-row').each(function(index,element){validationResetFields($(element).find('.value-field'))});dataTable.clear();if(typeof(saveAllButton)!="undefined"){saveAllButton.hide()}
if(typeof(editAllButton)!="undefined"){editAllButton.show()}
var rows=$.parseHTML($.trim(html));$(rows).each(function(index,element){if($(element).context.nodeName!="#text"){var row=dataTable.row.add($(element)).draw(!1).node();activateJs($(row))}});applyHook('saveAllRowsRequestDone',{"table":table});$.unblockUI()});request.fail(function(jqXHR,textStatus){$.unblockUI();alert("Request failed: "+textStatus)})};function editModal(event){var $row=$(event.target).closest('tr');var $modal=$('#edit-modal');applyHook('editModalStart',{"$modal":$modal,"$row":$row});var id=$row.attr('data-id');$.blockUI({message:''});var request=$.ajax({method:"GET",url:urls.urlAjaxEditableModalGet.replace("_id_",id),cache:!1});request.done(function(html){$modal.html(html);validationResetFields($modal.find('.value-field'));activateJs($modal);$modal.trigger('create');validationAddFields($modal.find('.value-field'));focusInput($modal);applyHook('editModalRequestDone',{"$modal":$modal});$.unblockUI()});request.fail(function(jqXHR,textStatus){$.unblockUI();alert("Request failed: "+textStatus)})}
function saveModal(event,recursive){if(typeof(recursive)=="undefined"){recursive=!1}
var $modal=$('#edit-modal');applyHook('saveModalStart',{"$modal":$modal});if(recursive==!1){$.blockUI({message:''})}
...
var id=$(event.target).attr('data-id');var $row=table.find('tr[data-id="'+id+'"]');var putData={};$modal.find('.value-field').each(function(index,element){if(!($(element).is(':checkbox'))||$(element).is(':checked')){putData[$(element).attr('name')]=$(element).val()}});var request=$.ajax({method:"PUT",url:urls.urlAjaxModalPut.replace("_id_",id),cache:!1,data:putData});request.done(function(html){validationResetFields($modal.find('.value-field'));var $rowIndex=dataTable.row($row).index();$html=$($.parseHTML($.trim(html)));$dRow=dataTable.row.add($html);dataTable.context[0].aoData[$rowIndex]=dataTable.context[0].aoData[$dRow.index()];$dRow.remove();dataTable.draw(!1);$row=$(dataTable.row($rowIndex).node());$("#edit-modal").niftyModal("hide");applyHook('saveModalRequestDone',{"$modal":$modal});activateJs($row);$.unblockUI()});request.fail(function(jqXHR,textStatus){$.unblockUI();alert("Request failed: "+textStatus)})}
function addNewModal(event){var $modal=$('#new-modal');applyHook('addNewModalStart',{"$modal":$modal});$.blockUI({message:''});var request=$.ajax({method:"GET",url:urls.urlAjaxNewModalGet,cache:!1});request.done(function(html){$modal.html(html);activateJs($modal);validationAddFields($modal.find('.value-field'));focusInput($modal);applyHook('addNewModalRequestDone',{"$modal":$modal});$.unblockUI()});request.fail(function(jqXHR,textStatus){$.unblockUI();alert("Request failed: "+textStatus)})}
function saveNewModal(event,recursive){if(typeof(recursive)=="undefined"){recursive=!1}
var $modal=$('#new-modal');applyHook('saveNewModalStart',{"$modal":$modal});if(recursive==!1){$.blockUI({message:''})}
...
$.blockUI({message:''});var putData={};$modal.find('.value-field').each(function(index,element){if(!($(element).is(':checkbox'))||$(element).is(':checked')){putData[$(element).attr('name')]=$(element).val()}});var request=$.ajax({method:"POST",url:urls.urlAjaxModalPost,data:putData,cache:!1});request.done(function(html){$modal.each(function(index,element){validationResetFields($(element).find('.value-field'))});var newRows=$.parseHTML($.trim(html));var rows=[];$(newRows).each(function(index,element){if($(element).context.nodeName!="#text"){var row=dataTable.row.add($(element)).draw(!1).node();rows[index]=$(row);activateJs($(row))}});$("#new-modal").niftyModal("hide");applyHook('saveNewModalRequestDone',{"$modal":$modal,"rows":rows});$.unblockUI()});request.fail(function(jqXHR,textStatus){$.unblockUI();alert("Request failed: "+textStatus)})};function sortRows(event){if(sortMode=="view"){table.prev().find('.dataTables_filter').hide();var columnIndex=applyFilter('sortRowsColumnIndex',0,{});dataTable.order([columnIndex,'asc']).draw();sortMode="sort";sortButton.html('Save Sort Order');if(typeof(cancelSortButton)!="undefined"){cancelSortButton.show()}
...
function disableSorting(){var columnIndex=applyFilter('sortRowsColumnIndex',0,{});var order=dataTable.order();if(order.length==1){if(order[0][0]!=columnIndex||order[0][1]!='asc'){dataTable.order([[columnIndex,"asc"]]).draw()}}else{dataTable.order([[columnIndex,"asc"]]).draw()}};function validationIsValid($container,recursive){if(typeof(recursive)=="undefined"){recursive=!1}
...
function validationResetFields($fields){if(typeof(formValidation)!="undefined"){$fields.each(function(index,element){formValidation.resetField($(element))})}};return{form:form,table:table,createInitButton:createInitButton,formValidation:formValidation,dataTable:dataTable,urls:urls}}