...
var sortButton=config.sortButton;var formValidation;table.on('draw.dt',function(){table.find('tbody td[data-colspan]').each(function(index,element){var colspan=$(element).attr('data-colspan');if($(element).attr('colspan')!=colspan){for(i=0;i<(colspan-1);i++){$(element).next().remove()}
$(element).attr('colspan',colspan)}})});var dataTable=table.DataTable(config.datatableArgs);var sortMode='view';if(typeof(form)!="undefined"){formValidation=form.data('formValidation');if($('[id='+form.attr('id')+']').length>1){console.warn('Form Validation will have problems, duplicate IDs detected!')}}
...
function sortRows(event){if(sortMode=="view"){dataTable.order([0,'asc']).draw();sortMode="sort";$(event.target).html('Save Sort Order');table.find('tbody').addClass('sort-body');table.find('tbody tr td:last-of-type').addClass('sort-column');table.find('tbody').sortable({helper:function(e,tr){var $originals=tr.children();var $helper=tr.clone();$helper.children().each(function(index,element){$(element).width($originals.eq(index).width())});$helper.find(".sort-column").removeClass('sort-column');return $helper}});table.find('tbody').sortable("enable");table.find('tbody').disableSelection()}else{sortMode="view";$(event.target).html('Change Sort Order');table.find('tbody').removeClass('sort-body');table.find('tbody tr td:last-of-type').removeClass('sort-column');table.find('tbody').sortable("disable");table.find('tbody').enableSelection();var sortable=table.find('tbody').sortable("instance");var ids=sortable.toArray({attribute:"data-id"});var data=applyFilter('sortRowsDataFilter',{ids:ids},{sortable:sortable});$.blockUI({message:''});var request=$.ajax({method:"PUT",url:urls.urlAjaxSortPut,cache:!1,data:data});request.done(function(html){$.unblockUI()});request.fail(function(jqXHR,textStatus){$.unblockUI();alert("Request failed: "+textStatus)})}}
