var App = function () {
    ...
    return {
        ...
        activate: function(parent) {
            /*Datepicker UI*/
            parent.find( ".ui-datepicker" ).datepicker();
            
            /* DateRange Picker */
            parent.find( ".daterangepicker" ).daterangepicker();
            
            /*Tooltips*/
            if(config.tooltip){
                parent.find('.ttip, [data-toggle="tooltip"]').tooltip();
            }
            
            /*Popover*/
            if(config.popover){
                parent.find('[data-popover="popover"]').popover();
            }

            /*NanoScroller*/      
            if(config.nanoScroller){
                //parent.find(".nscroller").nanoScroller();     
            }
            
            /*Nestable Lists*/
            if(config.nestableLists){
                parent.find('.dd').nestable();
            }
            ...
            /*DateTime Picker*/
            if(config.dateTime){
                parent.find(".datetime").datetimepicker({autoclose: true});
            }
            
            /*Time Picker*/
            if(config.time){
                parent.find(".timepicker").timepicker({ 'scrollDefault': 'now' });
            }
            ...
            if(config.select2){
             ...
             /*Tags*/
            if(config.tags){
                parent.find(".tags").select2({tags: 0,width: '100%'});
            }
            
             /*Slider*/
            if(config.slider){
                parent.find('.bslider').slider();     
            }
            ...
            parent.find('.md-trigger').modalEffects();

            /*Bind plugins on hidden elements*/
            if(config.hiddenElements){
                /*Dropdown shown event*/
                parent.find('.dropdown').on('shown.bs.dropdown', function () {
                    //parent.find(".nscroller").nanoScroller();
                });
                    
                /*Tabs refresh hidden elements*/
                parent.find('.nav-tabs').on('shown.bs.tab', function (e) {
                    //parent.find(".nscroller").nanoScroller();
                });
            }
        },
